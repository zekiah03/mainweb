...
function LayerDiagram() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {LAYERS.map((layer) => (
        <div key={layer.id} style={{ display: 'flex', gap: 14, padding: '14px 16px', borderRadius: 12, border: `1px solid ${layer.color}22`, background: layer.bg, flexWrap: 'wrap' }}>
          ...
          <div style={{ alignSelf: 'center', flexShrink: 0, fontSize: 10, color: layer.color, padding: '4px 9px', borderRadius: 6, border: `1px solid ${layer.color}30`, background: `${layer.color}10`, minWidth: 72, textAlign: 'center' }}>{layer.scale}</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, alignItems: 'center', flex: 1 }}>
            {layer.axes.map((ax) => (
              <div key={ax.id} style={{ padding: '7px 11px', borderRadius: 9, border: `1px solid ${layer.color}28`, background: `${layer.color}0a` }}>
...
function SigmaSection() {
  return (
    <div style={{ padding: '24px', borderRadius: 14, border: '1px solid rgba(96,165,250,0.2)', background: 'rgba(96,165,250,0.05)' }}>
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        <svg viewBox="0 0 130 130" width="110" height="110" style={{ flexShrink: 0 }}>
          <circle cx="65" cy="65" r="52" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="14" strokeDasharray="245 80" strokeDashoffset="-40" strokeLinecap="round" />
          <circle cx="65" cy="65" r="52" fill="none" stroke="rgba(96,165,250,0.5)" strokeWidth="14" strokeDasharray="100 225" strokeDashoffset="-40" strokeLinecap="round" />
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.52)', lineHeight: 1.75 }}>感情変化のしやすさを表すスカラー値。<strong style={{ color: 'rgba(96,165,250,0.8)', fontWeight: 500 }}>atlas</strong> アプリの HSP-8 問診で推定し、全軸のベイズ更新レートに乗算されます。σ_u が大きいほど新しい観測が素早く反映されます。</div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
        {[{ val: '0.7', label: '低感受性', sub: '変化がゆっくり', color: 'rgba(96,165,250,0.8)' }, { val: '1.0', label: '標準 (初期値)', sub: 'デフォルト', color: 'rgba(255,255,255,0.65)' }, { val: '1.4', label: '高感受性', sub: '変化が速い', color: 'rgba(167,139,250,0.8)' }].map(({ val, label, sub, color }) => (
          <div key={val} style={{ flex: 1, padding: '12px 0', textAlign: 'center', borderRadius: 10, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)' }}>
  ...
  return (
    <div style={{ padding: '24px 26px', borderRadius: 14, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', ...mono }}>
      <div style={{ marginBottom: 22 }}>
        ...
        <div style={note}>α_app: アプリ信頼度 (0.5–1.0)　σ_u: 個体感受性 (0.7–1.4)　β_axis: 軸の可変性 (0.0–1.0)</div>
      </div>
      <div style={div1} />
      <div style={{ marginBottom: 22 }}>
        ...
        <div style={note}>δ_obs: アプリから届いた観測値ベクトル</div>
      </div>
      <div style={div1} />
      <div style={{ marginBottom: 22 }}>
      ...
      <div>
        ...
        <div style={note}>各軸に半減期あり。Layer I: τ = 365–3650日。Layer IV: τ = 30–90日。古い観測は自然に薄れます。</div>
      </div>
    </div>
  )
}
...
function AppAxisMap() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {APPS_MAP.map((app) => (
        <div key={app.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 16px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {app.axes.map((ax) => (
              <span key={ax} style={{ fontSize: 11, fontWeight: 700, color: AXIS_COLORS[ax] ?? 'rgba(255,255,255,0.5)', padding: '2px 8px', borderRadius: 5, border: `1px solid ${AXIS_COLORS[ax] ?? 'rgba(255,255,255,0.2)'}35`, background: `${AXIS_COLORS[ax] ?? 'rgba(255,255,255,0.1)'}10` }}>{ax}</span>
            ))}
          </div>
        </div>
      ))}
...
function HypothesesSection() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {HYPOTHESES.map((h) => (
        <div key={h.id} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '14px 18px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)' }}>
...
function Morpho28Diagram() {
  const positive = ['joy','sadness','anger','fear','surprise','disgust','love','gratitude','pride','hope','relief','curiosity','awe','flow','elevation','contentment']
  const negative = ['anxiety','grief','disappointment','regret','boredom','loneliness','frustration','nostalgia','jealousy','envy','shame','guilt']
  return (
    <div style={{ padding: '20px', borderRadius: 14, border: '1px solid rgba(52,211,153,0.18)', background: 'rgba(52,211,153,0.04)' }}>
      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', marginBottom: 14 }}>G・H・I 軸は同じ <strong style={{ color: 'rgba(52,211,153,0.9)', fontWeight: 500 }}>28感情次元</strong>を共有。G = 強度、H = 感受性、I = 持続。</div>
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontSize: 9, letterSpacing: '0.2em', color: 'rgba(52,211,153,0.6)', marginBottom: 8 }}>POSITIVE (16)</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {positive.map((e) => (<span key={e} style={{ fontSize: 9.5, color: 'rgba(52,211,153,0.7)', padding: '2px 7px', borderRadius: 5, border: '1px solid rgba(52,211,153,0.15)', background: 'rgba(52,211,153,0.06)' }}>{e}</span>))}
          </div>
        </div>
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontSize: 9, letterSpacing: '0.2em', color: 'rgba(251,191,36,0.6)', marginBottom: 8 }}>CHALLENGING (12)</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {negative.map((e) => (<span key={e} style={{ fontSize: 9.5, color: 'rgba(251,191,36,0.65)', padding: '2px 7px', borderRadius: 5, border: '1px solid rgba(251,191,36,0.15)', background: 'rgba(251,191,36,0.05)' }}>{e}</span>))}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '52px 24px 110px' }}>
      <div className="fsi" style={{ animationDelay: '0.04s', marginBottom: 64 }}>
        ...
        <h1 style={{ fontSize: 27, fontWeight: 300, margin: '0 0 14px', color: 'rgba(255,255,255,0.9)', letterSpacing: '-0.018em', lineHeight: 1.3 }}>デジタルツイン理論</h1>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.42)', lineHeight: 1.85, margin: 0, maxWidth: 480 }}>複数のアプリから収集された行動・感情・価値データが、<br />どのようにして「もうひとりの自分」を形成するか——その全体像。</p>
      </div>
      <div className="fsi" style={{ animationDelay: '0.10s', ...s }}>
        <SectionLabel>01 · システム構成</SectionLabel>
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.42)', lineHeight: 1.8, margin: '0 0 20px' }}>衛星アプリが送る生データは Contribution API を経由し、プロジェクション関数が「どの軸のどの次元に何の影響があるか」を計算します。その結果を使い Morpho Profile をベイズ更新します。更新の強さは個体感受性 σ_u で変調されます。</p>
        <FlowDiagram />
      </div>
      <div className="fsi" style={{ animationDelay: '0.18s', ...s }}>
        <SectionLabel>02 · 4層アーキテクチャ</SectionLabel>
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.42)', lineHeight: 1.8, margin: '0 0 20px' }}>自己は4つの時間スケールで記述されます。Layer I ほど変化が遅く本質的。Layer IV ほど日々変動する表面的な表現です。各軸はガウス分布 μ (平均ベクトル) と σ² (分散) で保持されます。</p>
        <LayerDiagram />
      </div>
      <div className="fsi" style={{ animationDelay: '0.24s', ...s }}>
        <SectionLabel>03 · 28感情次元 (Layer III)</SectionLabel>
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.42)', lineHeight: 1.8, margin: '0 0 20px' }}>感情を28種に分類し、それぞれの「強度 G」「感受性 H」「持続 I」を独立して記録します。同じ感情でも「すぐ強く感じるが短い」「じわじわ長く続く」など個人差が現れます。</p>
        <Morpho28Diagram />
      </div>
      ...
      <div className="fsi" style={{ animationDelay: '0.36s', ...s }}>
        ...
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.42)', lineHeight: 1.8, margin: '0 0 20px' }}>各軸はガウス分布として表現されます。新しい観測が届くたびに平均 μ と分散 σ² が更新されます。分散が小さいほど「確信度が高い」ことを意味します。</p>
        <FormulaSection />
      </div>
      <div className="fsi" style={{ animationDelay: '0.42s', ...s }}>
        <SectionLabel>06 · アプリ → 軸マッピング</SectionLabel>
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.42)', lineHeight: 1.8, margin: '0 0 16px' }}>各アプリが主にどの軸を更新するかを示します。 <span style={{ color: '#a78bfa' }}>■</span> Layer I <span style={{ color: '#60a5fa' }}>■</span> Layer II <span style={{ color: '#34d399' }}>■</span> Layer III <span style={{ color: '#fbbf24' }}>■</span> Layer IV</p>
        <AppAxisMap />
      </div>
      <div className="fsi" style={{ animationDelay: '0.48s', ...s }}>
        <SectionLabel>07 · 仮説システム SUST-1〜10</SectionLabel>
        <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.42)', lineHeight: 1.8, margin: '0 0 20px' }}>Morpho Profile の特定パターンが閾値を超えると「仮説が発火」し、インサイトカードが生成されます。仮説は新しい寄与が届くたびにリアルタイムで再評価されます。</p>
        <HypothesesSection />
      </div>
    </div>
  )
